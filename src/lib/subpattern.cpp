...
#pragma omp critical
        {
          if (extr_id.x == 29 && extr_id.y == 147 && int(c_i.p.y) == 1005)  {
            printf("add %fx%f (%dx%d) from %fx%f (%dx%d) and %fx%f\n", c_i.p.x,c_i.p.y,extr_id.x,extr_id.y,c.p.x,c.p.y,c.id.x,c.id.y,second.x,second.y);
              imwrite("fitted.tif", *paint);
            abort();
          }
